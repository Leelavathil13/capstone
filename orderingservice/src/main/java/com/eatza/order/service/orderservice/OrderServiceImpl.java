...
import com.eatza.order.feignclient.MyFeignClient;
...
public class OrderServiceImpl implements OrderService {
	...
	public Order placeOrder(OrderRequestDto orderRequest) throws OrderException {
		...
		for(OrderedItemsDto itemDto: itemsDtoList) {
			...
			try {
				...
				ItemFetchDto item =(ItemFetchDto)myfeignClient.feignclientResponse(itemDto.getItemId());
	...
	public OrderUpdateResponseDto updateOrder(OrderUpdateDto orderUpdateRequest) throws OrderException {
		...
		for(OrderedItemsDto itemDto: itemsDtoList) {
			...
			//restTemplate.setMessageConverters(messageConverters);
			try {
				ItemFetchDto item =myfeignClient.feignclientResponse(itemDto.getItemId());
				
				/*
				 * ItemFetchDto item =
				 * restTemplate.getForObject(restaurantServiceItemUrl+itemDto.getItemId(),
				 * ItemFetchDto.class);
				 */if(item==null ) {
